...
- Update Rule: `host = @request.auth.id || participants.id ?= @request.auth.id`
- Delete Rule: `host = @request.auth.id`
...
7. `allow_participant_queueing` (Bool): If true, participants can add to the queue. Default to true.
